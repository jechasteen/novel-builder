#!/bin/env bash

CHAPTERS=chapters/*.md
BUILD=build

set -e

# https://github.com/jasperes/bash-yaml
# Based on https://gist.github.com/pkuczynski/8665367
parse_yaml() {
    local yaml_file=$1
    local prefix=$2
    local s
    local w
    local fs

    s='[[:space:]]*'
    w='[a-zA-Z0-9_.-]*'
    fs="$(echo @ | tr @ '\034')"

    (
        sed -e '/- [^\“]'"[^\']"'.*: /s|\([ ]*\)- \([[:space:]]*\)|\1-\'$'\n''  \1\2|g' |
            sed -ne '/^--/s|--||g; s|\"|\\\"|g; s/[[:space:]]*$//g;' \
                -e 's/\$/\\\$/g' \
                -e "/#.*[\"\']/!s| #.*||g; /^#/s|#.*||g;" \
                -e "s|^\($s\)\($w\)$s:$s\"\(.*\)\"$s\$|\1$fs\2$fs\3|p" \
                -e "s|^\($s\)\($w\)${s}[:-]$s\(.*\)$s\$|\1$fs\2$fs\3|p" |
            awk -F"$fs" '{
            indent = length($1)/2;
            if (length($2) == 0) { conj[indent]="+";} else {conj[indent]="";}
            vname[indent] = $2;
            for (i in vname) {if (i > indent) {delete vname[i]}}
                if (length($3) > 0) {
                    vn=""; for (i=0; i<indent; i++) {vn=(vn)(vname[i])("_")}
                    printf("%s%s%s%s=(\"%s\")\n", "'"$prefix"'",vn, $2, conj[indent-1], $3);
                }
            }' |
            sed -e 's/_=/+=/g' |
            awk 'BEGIN {
                FS="=";
                OFS="="
            }
            /(-|\.).*=/ {
                gsub("-|\\.", "_", $1)
            }
            { print }'
    ) <"$yaml_file"
}

unset_variables() {
    # Pulls out the variable names and unsets them.
    #shellcheck disable=SC2048,SC2206 #Permit variables without quotes
    local variable_string=($*)
    unset variables
    variables=()
    for variable in "${variable_string[@]}"; do
        tmpvar=$(echo "$variable" | grep '=' | sed 's/=.*//' | sed 's/+.*//')
        variables+=("$tmpvar")
    done
    for variable in "${variables[@]}"; do
        if [ -n "$variable" ]; then
            unset "$variable"
        fi
    done
}

create_variables() {
    local yaml_file="$1"
    local prefix="$2"
    local yaml_string
    yaml_string="$(parse_yaml "$yaml_file" "$prefix")"
    unset_variables "${yaml_string}"
    eval "${yaml_string}"
}
# END parse_yaml

create_variables novel.yml

help() {
    echo -e "novel-builder usage:\t./build.sh [type]"
    echo -e "\t\tmanuscript - a double-spaced submission-ready manuscript"
    echo -e "\t\tepub - ereader format"
}

check_build_dir() {
    if [[ ! -d $BUILD ]]; then
        mkdir $BUILD
    fi
}

manuscript() {
    check_build_dir

    awk 'FNR==1 && NR!=1 {print "\n\n"}{print}' $CHAPTERS |
        awk '{ gsub(/^---/, "\\begin{center}\n* * *\\end{center}"); print }' |
        pandoc -o $BUILD/$filename-manuscript.pdf \
            -f markdown+raw_tex \
            --metadata-file=manuscript.yml &&
        echo "Manuscript completed: $BUILD/$filename-manuscript.pdf"
}

epub() {
    check_build_dir

    awk 'FNR==1 && NR!=1 {print "\n\n"}{print}' $CHAPTERS |
        pandoc -o $BUILD/$filename.epub \
            --metadata-file=epub.yml &&
        echo "EPUB completed: $BUILD/$filename.epub"
}

if [[ -z $1 ]]; then
    help
    exit 1
elif [[ $1 = "manuscript" ]]; then
    manuscript && exit 0
elif [[ $1 = "epub" ]]; then
    epub && exit 0
fi
